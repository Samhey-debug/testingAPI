...
// Main handler function for creating channels
module.exports = async (req, res) => {
    ...
    try {
        ...
        const categoryMap = {};
        await Promise.all(sourceChannels.filter(c => c.type === 4).map(async (channel) => { // type 4 indicates a category
            try {
        ...
        await Promise.all(sourceChannels.filter(c => c.type !== 4).map(async (channel) => { // Filters out categories
            const payload = {
                name: channel.name,
                type: channel.type,
                position: channel.position,
